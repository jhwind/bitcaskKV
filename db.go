...
func Open(options Options) (*DB, error) {
	...
	if options.IndexType != BPlusTree {
		...
		// 为什么需要重置？因为只是启动的时候用的 mmap，还没实现 write sync，后面要换回来
		// 为什么不写数据呢？加大内存的开销，不简洁
		// 重置 IO 类型为标准文件 IO
...
func (db *DB) appendLogRecord(logRecord *data.LogRecord) (*data.LogRecordPos, error) {
	// 当前活跃文件是否存在，数据库在没有写入的时候是没有文件生成的
	// 如果为空则初始化数据文件
	if db.activeFile == nil {
...
func (db *DB) Delete(key []byte) error {
	...
	// 先从内存索引中查找 key 是否存在，不存在返回
	if pos := db.index.Get(key); pos == nil {
		return nil
	}
	// 检查 logRecord 标识其是否被删除
...
func (db *DB) Close() error {
	...
	// 主要是 b+tree 的索引关闭，b+树的话索引其实也是 bbolt 数据库实例，它只允许一个线程访问，所以我们关闭数据库的时候，这个也要关闭
