...
func (db *DB) Merge() error {
	...
	// 不需要每次 sync，可以自己写入的时候每次 sync，中途发生错误的话 merge 不会成功，最后再手动 sync，中途发生任何错误认为 merge 不成功即可
...
func (db *DB) loadMergeFiles() error {
	...
	// 通过有没有 MergeFinishedFileName 这个文件判断是否 merge 完成
	...
	for _, fileName := range mergeFileNames {
		// /temp/bitcaskKV-merge/00.data
		// /temp/bitcaskKV/00.data
...
// 从 mergeFinishedFile 文件中读取 NonMergeFileId
func (db *DB) getNonMergeFileId(dirPath string) (uint32, error) {
	...
	mergeFinishedFile.IoManager.Close()
