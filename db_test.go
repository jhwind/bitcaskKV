...
func NewDb(dirPath string) (*DB, error) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", dirPath)
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	return Open(opts)
}
func BenchmarkPut(b *testing.B) {
	db, err := NewDb("bitcaskKV-put")
	assert.Nil(b, err)
	defer destroyDB(db)
...
// 	for i := 0; i < b.N; i++ {
// 		rdb.Set(ctx, string(utils.GetTestKey(i)), string(utils.RandomValue(128)), 0)
// 	}
// }

func BenchmarkGet(b *testing.B) {
	db, err := NewDb("bitcaskKV-get")
	assert.Nil(b, err)
	defer destroyDB(db)
...
func TestDB_Put(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-put")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := Open(opts)
	defer destroyDB(db)
...
func TestDB_Get(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-get")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := Open(opts)
	defer destroyDB(db)
...
func TestDB_Delete(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-delete")
	opts.DirPath = dir
	opts.DataFileSize = 64 * 1024 * 1024
	db, err := Open(opts)
	defer destroyDB(db)
...
func TestDB_ListKeys(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-list-keys")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Fold(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-fold")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Close(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-close")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Sync(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-sync")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_FileLock(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-filelock")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Stat(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-stat")
	opts.DirPath = dir
	db, err := Open(opts)
	// defer destroyDB(db)
	assert.Nil(t, err)
...
func TestDB_Backup(t *testing.T) {
	opts := DefaultOptions
	dir, _ := os.MkdirTemp("", "bitcaskKV-backup")
	opts.DirPath = dir
	db, err := Open(opts)
	defer destroyDB(db)
	assert.Nil(t, err)
	...
	backupDir, _ := os.MkdirTemp("", "bitcaskKV-backup-test")
